...
namespace LessJie.DataUtils
{
    public class DataOperateHelper
    {
        ...
        {
            string _text = ConvertHelper.ToObjectString(text);
        ...
        {
            string text = ConvertHelper.ToObjectString(numricString).Trim();
            string returnText = String.Empty;
            char[] chars = text.ToCharArray();
            for (int i = 0; i < chars.Length; i++)
            {
                if (chars[i].ToString() == "+" || chars[i].ToString() == "-" || DataValidate.IsDouble(chars[i].ToString()))
                    returnText += chars[i].ToString();
            }
            return returnText;
        }
            ...
            try
            {
                if (fi.Exists)
                {
                    ...
                    do
                    {
                        ...
                        if (lowerUpper)
                        {
                            if (ConvertHelper.ToObjectString(g).Trim() == ConvertHelper.ToObjectString(lineText).Trim())
                            {
                                b = true;
                                break;
                            }
                        ...
                        {
                            if (ConvertHelper.ToObjectString(g).Trim().ToLower() == ConvertHelper.ToObjectString(lineText).Trim().ToLower())
                            {
                                b = true;
                                break;
                            }
        ...
        private static void SearchChild(ArrayList array, DataTable table, string columnName, string parentColumnName, string inputString, string compareString)
        {
            ...
            {
                if (ConvertHelper.ToObjectString(view[index][columnName]).ToLower() == compareString.Trim().ToLower())
                {
                    array.Add("1");
                    break;
                }
                ...
                {
                    SearchChild(array, table, columnName, parentColumnName, ConvertHelper.ToObjectString(view[index][columnName]), compareString);
                }
            }
        }
